...
std::vector<std::shared_ptr<Node>> CascadeBuilder::HandleFail(const size_t current_depth) {
    ...
    for (size_t conj_idx = 0; conj_idx < rf_data_.conj_count; conj_idx++) {
        ...
        for (size_t pred_idx = 0; pred_idx < rf_data_.pred_count.at(conj_idx); pred_idx++) {
            for (size_t rf_idx = 0; rf_idx < rf_data_.rf_count.at(conj_idx).at(pred_idx); rf_idx++) {
                if (used_rfs_.at(conj_idx).at(pred_idx).test(rf_idx)) {
                    continue;
                }

                used_rfs_.at(conj_idx).at(pred_idx).set(rf_idx);
                ...
                used_rfs_.at(conj_idx).at(pred_idx).reset(rf_idx);
            }
        }
...
std::vector<std::shared_ptr<Node>> CascadeBuilder::HandleSuccess(const size_t current_depth, const size_t conj_idx) {
    size_t const conj_left = disjunction_.conjunctions.size() - used_conjunctions_.count();
    size_t const free = kMaxDepth - current_depth - conj_left;
